...
/**
 * Servlet implementation class ExclusiveDoAddAmuletServlet
 */
public class ExclusiveDoAddAmuletServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
            ...
            for (FileItem item : items) {
                if (item.isFormField()) {
                    if (item.getFieldName().equals("amulet_name")) {
                        amulet_name = new String(item.getString().getBytes("iso-8859-1"),"UTF-8");//new String (item.getString().getBytes ("iso-8859-1"), "UTF-8");
        ...
        AmuletMasterEntity entity = new AmuletMasterEntity();
        ...
        entity.setDefectDes1(defect_des_1);
        ...
        int amuletCode = AmuletMasterDAO.getInstance().saveAmuletMaster(entity);

        for (FileItem fi:fileUpload) {
            if (StringUtils.isNotEmpty(fi.getName())) {
                String fname = AmuletUtils.uploadImage(fi, String.valueOf(amuletCode), filePath);
                ...
                if (request.getSession().getAttribute("amulet_entity") != null) {
                    AmuletMasterEntity amuletMasterEntity = (AmuletMasterEntity) request.getSession().getAttribute("amulet_entity");
                    if (fi.getFieldName().equals("pic1")) {
        ...
        // update amulet info with amulet picture file path
        ...
        if (updateSuccess) {
            ...
            if (user.getRole().equals(AmuletConstants.ADMIN_TYPE)) {
                response.sendRedirect("amuletinqservlet"); //logged-in page
            } else if (user.getRole().equals(AmuletConstants.EXCV_TYPE)) {
                response.sendRedirect("ex_manage_amulet"); //logged-in page
            }
            request.getSession().setAttribute("uploadamulet","success");
        } else {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String amuletId = "";
        amuletId = request.getParameter("amuletId");
        // get amulet master details of selected list for preview/update
        AmuletMasterEntity amuletMasterEntity = AmuletMasterDAO.getInstance().getAmuletEntity(Integer.parseInt(amuletId));
        request.getSession().setAttribute("amulet_master_entity",amuletMasterEntity);
        response.sendRedirect("addamuletpage"); //use the same page as add
    }
}